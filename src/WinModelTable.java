	...
	public WinModelTable() {
		...
		table.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				if(e.getKeyCode() == KeyEvent.VK_ENTER) {
					...
					System.out.println(temp + "예전>>" + sTemp);
				}	
			}
		});
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				int row = table.getSelectedRow();
				int col = table.getSelectedColumn();
				sTemp = table.getValueAt(row, col).toString();
				record = new String[7];
				for(int i=0; i<record.length; i++)
					record[i] = table.getValueAt(row, i).toString();
				tfName.setText(record[1]);
				tfKorean.setText(record[2]);
				tfEnglish.setText(record[3]);
				tfMath.setText(record[4]);
				tfTotal.setText(record[5]);
				tfAverage.setText(record[6]);
